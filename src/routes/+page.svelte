<script>
  ...
  //   const res = await fetch('bookmarklets/reviews.js');
...
// <p>{fileContent}</p>
  ...
  <div class="container mx-auto px-4 py-8">
    <header class="flex justify-between items-center mb-8">
      ...
      <button on:click={toggleTheme} class="p-2 rounded-md bg-secondary text-secondary-foreground">
        Toggle Theme
      </button>
    </header>
    <main>
      <div class="mb-4">
        <h2 class="text-xl font-semibold mb-2">Reviews Bookmarklet</h2>
        <div class="relative">
          <pre id="code-block" class="bg-muted text-muted-foreground p-4 rounded-lg overflow-x-auto">
            <code>{reviewsCode}</code>
          </pre>
          <button on:click={copyCode} class="absolute top-2 right-2 p-2 bg-primary text-primary-foreground rounded-md" aria-label="Copy Code">
             {#if isCopied} {@html copied} {:else} {@html copy} {/if}
          </button>
        </div>
      </div>

      <div class="mb-4">
        <h2 class="text-xl font-semibold mb-2">QR Bookmarklet</h2>
        <div class="relative">
          <pre id="code-block" class="bg-muted text-muted-foreground p-4 rounded-lg overflow-x-auto">
            <code>{qrCode}</code>
          </pre>
